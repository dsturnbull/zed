...
impl From<latest::CodeLabel> for CodeLabel {
    fn from(value: latest::CodeLabel) -> Self {
        Self {
            code: value.code,
            spans: value.spans.into_iter().map(Into::into).collect(),
            filter_range: value.filter_range,
...
impl From<latest::CodeLabelSpan> for CodeLabelSpan {
    fn from(value: latest::CodeLabelSpan) -> Self {
        match value {
            latest::CodeLabelSpan::CodeRange(range) => Self::CodeRange(range),
            latest::CodeLabelSpan::Literal(literal) => Self::Literal(literal.into()),
        }
    }
}

impl From<latest::CodeLabelSpanLiteral> for CodeLabelSpanLiteral {
    fn from(value: latest::CodeLabelSpanLiteral) -> Self {
        Self {
            text: value.text,
            highlight_name: value.highlight_name,
        }
