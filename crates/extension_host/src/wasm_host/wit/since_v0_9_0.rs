...
use gpui::{BackgroundExecutor, RenderImage, SharedString};
...
wasmtime::component::bindgen!({
    ...
    with: {
         ...
         "zed:extension/frame-buffer/frame-buffer": ExtensionFrameBuffer,
    },
});
...
pub type ExtensionFrameBuffer = HostFrameBuffer;
...
impl From<LaunchRequest> for task::LaunchRequest {
    fn from(value: LaunchRequest) -> Self {
        Self {
            program: value.program,
            cwd: value.cwd.map(|path| path.into()),
            args: value.args,
...
#[async_trait]
impl frame_buffer::Host for WasmState {}

impl frame_buffer::HostFrameBuffer for WasmState {
    async fn create(
        &mut self,
        width: u32,
        height: u32,
    ) -> wasmtime::Result<Result<Resource<ExtensionFrameBuffer>, String>> {
        if width == 0 || height == 0 {
            return Ok(Err("frame buffer dimensions must be non-zero".to_string()));
        }

        let pixel_count = (width as usize)
            .checked_mul(height as usize)
            .and_then(|count| count.checked_mul(4));
        let total_bytes = match pixel_count {
            Some(total_bytes) => total_bytes,
            None => return Ok(Err("frame buffer dimensions too large".to_string())),
        };

        let shared = self.host.frame_buffer_registry.create(width, height);
        let buffer = HostFrameBuffer {
            width,
            height,
    ...
    async fn width(
        &mut self,
        self_: Resource<ExtensionFrameBuffer>,
    ) -> wasmtime::Result<u32> {
        let buffer = self.table.get(&self_)?;
        Ok(buffer.width)
    }
    ...
    async fn write(
        ...
        let buffer = self.table.get_mut(&self_)?;
        let offset = offset as usize;
        let total_bytes = (buffer.width as usize) * (buffer.height as usize) * 4;

        if offset.saturating_add(data.len()) > total_bytes {
            return Ok(Err(format!(
                "write out of bounds: offset {} + length {} exceeds buffer size {}",
                offset,
                data.len(),
                total_bytes,
            )));
        ...
        buffer.pixels[offset..offset + data.len()].copy_from_slice(&data);
        Ok(Ok(()))
    }
    ...
    async fn replace(
        ...
        if data.len() != total_bytes {
            return Ok(Err(format!(
                "replacement data length {} does not match buffer size {}",
                data.len(),
                total_bytes,
            )));
        }
    ...
    async fn resize(
        ...
        buffer.pixels = vec![0u8; total_bytes];
        Ok(Ok(()))
    }

    async fn drop(
        &mut self,
        representative: Resource<ExtensionFrameBuffer>,
    ) -> wasmtime::Result<()> {
        self.table.delete(representative)?;
        Ok(())
    }
}
                    ...
                    "context_servers" => {
                        ...
                        match settings {
                            ...
                            project::project_settings::ContextServerSettings::Http { .. } => {
                                bail!("remote context server settings not supported in 0.9.0")
                            }
                        }
                    }
                    _ => {
